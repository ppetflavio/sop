"""
update.py — Gera data.json a partir do Excel e opcionalmente faz push
Uso:
  python update.py                        # usa planilha/BaseHtmlv1.xlsx
  python update.py outra_planilha.xlsx    # usa arquivo especificado
"""
...
def build(xlsx_path):
    ...
    for emp_num, emp_name in EMPS.items():
        ...
        for cat,cd in d.groupby('Categoria'):
            cats.append({'Categoria':str(cat),
                'real':float(cd['Realizado Acumulado (Q)'].sum()),
                'meta':float(cd['Meta Mês Total (Q)'].sum()),
                'proj':float(cd['Projeção Venda (Q)'].sum()),
        ...
        dseg=d[d['Necessidade Compra Mês Seguinte (Q)']>0].copy()
        dseg=dseg.assign(_i=dseg['Necessidade Compra Mês Seguinte (Q)']*dseg['Preço Custo ($)']).sort_values('_i',ascending=False)
        ...
        for comp,cd in d.groupby('Comprador'):
            ...
            comps.append({'Comprador':comp,'skus':int(len(cd)),
                ...
                'receita':float((cd['Realizado Acumulado (Q)']*cd['Preço Praticado ($)']).sum()),
                'meta':float(cd['Meta Mês Total (Q)'].sum())})
        crit=d[(d['Status Estoque']=='Estoque Zerado')&(d['Status Venda']=='Acima Meta')].copy()
        crit=crit.assign(_r=crit['Projeção Venda (Q)']*crit['Preço Praticado ($)']).nlargest(50,'_r')
        criticos=[{'Produto':str(r['Produto']),'Descrição':str(r['Descrição']),'Marca':str(r['Marca']),
            'Categoria':str(r['Categoria']),'Comprador':str(r['Comprador']),
            'Projeção Venda (Q)':float(r['Projeção Venda (Q)']),'Necessidade Compra (Q)':float(r['Necessidade Compra (Q)']),
            'Receita_Risco':float(r['Projeção Venda (Q)']*r['Preço Praticado ($)']),
        ...
        result[str(emp_num)]={
            ...
            'nec_seg':ns,'peso_seg':f('Peso Total Kg (Necessidde compra Mês seguinte)'),
            'nec_val':fp('Necessidade Compra Mês Seguinte (Q)','Preço Custo ($)'),
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        # Procura em planilha/ ou na pasta atual
        candidates = list(Path('planilha').glob('*.xlsx')) if Path('planilha').exists() else []
        candidates += list(Path('.').glob('*.xlsx'))
        if not candidates:
            print("ERRO: Nenhum arquivo .xlsx encontrado.")
            print("Uso: python update.py arquivo.xlsx")
            sys.exit(1)
        xlsx = candidates[0]
    ...
    skus  = sum(v['skus'] for v in D.values())
    print(f"  {skus} SKUs | Nec. Compra Seg.: {total:.0f} un")
